...
|p. 110, P4.3 |  |  |1) The resistance at the drain should be $5~k\Omega$. 2) The problem should be first solved for the case of $V_{SB}=0$ (part a), and then the gain should be recomputed wit the bulk connected to the supply (part b). 2) The propblem statement should include: The circuit is biased such that no DC current flows into the resistor connected to the drain. Explain why channel-length modulation can be safely neglected in the calculation of $i_{out}/i_s$.|
